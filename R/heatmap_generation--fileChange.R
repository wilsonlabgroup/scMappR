...
heatmap_generation <- function(genesIn, comp, cex = 0.8, cellTypes = "ALL", pVal = 0.01, isPval=TRUE, isMax =F,  isBackground = F,reference = "C:/Users/Dustin Sokolowski/Desktop/romanov_wilcoxon_test_2.RData",  which_species = "human") {
  ...
  if(length(whichGenesInter) == 1) { # if only 1 gene is
    ...
    barplot(wilcoxon_rank_mat_t[whichGenesInter,], las = 2, main = rownames(wilcoxon_rank_mat_t)[whichGenesInter])
    dev.off()
  ...
  if(length(whichGenesInter) > 1) { # if > 1 genes are
    ...
    heatmap.2(wilcoxon_rank_mat_t[whichGenesInter,], Rowv = T, dendrogram = "column", col = myheatcol, scale = "row", trace = "none", margins = c(7,7),cexRow = cex, cexCol = 0.3 )
    dev.off()
