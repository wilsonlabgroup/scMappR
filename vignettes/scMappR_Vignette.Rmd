---
title: "Single Cell Mapper (scMappR) Vignette"
...
When `toSave == TRUE`, it is also required to set the directory where files can be saved. In this vignette, it is assumed that there is a directory called `vignette_dir` in the working directory. To change the directory where files are saved, change the `path` argument.
...
```{r library_scMappR, include=TRUE, echo = FALSE}
...
internal <- tissue_scMappR_internal(genes, "mouse", output_directory = "scMappR_Test_Internal", tissue = "hypothalamus", rda_path = rda_path1, toSave = TRUE, path = "vignette_dir")
...
The example below has `toSave = FALSE`, `up_and_downregulated = FALSE` and `internet = FALSE`. When running scMappR yourself, it is strongly recommended to set all to `TRUE`. toSave = TRUE allows for the printing of folders, images, and files onto a desktop/cluster. up_and_downregulated = TRUE repeats pathway analysis for up and down-regulated genes separately. internet = TRUE allows for the completion of pathway analysis altogether.
...
toOut <- scMappR_and_pathway_analysis(bulk_normalized, odds_ratio_in, 
                                      ...
                                      internet = TRUE, toSave = TRUE, path = "vignette_dir")
...
Without anything saved, there are still a multitude of relevant outputs to scMappR_and_pathway_analysis. It returns a list containing: \
  * cellWeighted_Foldchanges: A gene by cell-type matrix of cellWeighted_Foldchanges.
  * cellType_Proportions: A sample by cell-type matrix of cell-type proportions estimated with DeconRNA-seq.
  * leave_one_out_proportions: A gene by cell-type matrix containing the mean cell-type proportions of each cell-type after that gene was removed.
  * processed_signature_matrix: Only necessary when removing unknown cell-types or scaling (not recommended). Returns signature matrix with changes from `deconvolute_and_contextualize.
  * ProportionT.test: output of t-test's to measure cell-type proportion differences in case/control.

```{r load_vignette_example, include=FALSE, echo=FALSE}
...
toOut <- scMappR_and_pathway_analysis(bulk_normalized, odds_ratio_in, 
                                      ...
                                      internet = FALSE, toSave = FALSE, path = "vignette_dir")
...
#cell-weighted Fold Changes (cwFold-changes)
toOut$cellWeighted_Foldchanges[1:4,1:4]

#cell-type proportions
toOut$cellType_Proportions[1:4,1:4]

# mean cell-type proportions changed by removing gene
toOut$leave_one_out_proportions[1:4,1:4]

# Signature matrix (potentially after some internal processing)
toOut$processed_signature_matrix[1:4,1:4]

# t.test output of signature matrix
toOut$ProportionT.test[1:4,]
...
Assuming `toSave = T` and `up_and_downregulated = T`, `scMappR_and_pathway_analysis()` will also generate a folder in your current directory with a considerable amount of data/figures.
  ...
  * scMappR_vignette_reordered_transcription_factors.Rdata: .Rdata file containing a list of outputs from g:profileR or gprofiler2 that are transcription factors enriched after re-ordering.
  * scMappR_vignette_reordered_pathways: .Rdata file containing list of outputs from g
  :profileR or gprofiler2 that are  GO:BP, REAC, and KEGG enrichments after re-ordering.
 ...
 * scMappR_vignette_bulk_transcription_factors.Rdata: g:ProfileR enrichment of bulk transcription factors.
 * scMappR_vignette_bulk_pathways.Rdata: g:ProfileR enrichment of bulk GO:BP, REAC, and KEGG.
...
```{r tissue_ct_enrichment_example, fig.show='hide'}
...
tst1 <- process_dgTMatrix_lists(toProcess, name = "testPropcess", species_name = -9, naming_preference = "eye", rda_path = "",  toSave = TRUE, saveSCObject = TRUE, path = "vignette_dir")
...
```{r load_processing_vignette_example, include=FALSE,echo=FALSE}

data(sm)

toProcess <- list(example = sm)

tst1 <- process_dgTMatrix_lists(toProcess, name = "testPropcess", species_name = -9, naming_preference = "eye", rda_path = "", toSave = FALSE)

```


```{r processed_scRNAseq_look}

#P value
head(tst1$wilcoxon_rank_mat_t[,1:3])

#Odds Ratio
head(tst1$wilcoxon_rank_mat_or[,1:3])
...
myheatcol <- grDevices::colorRampPalette(c("lightblue", "white", "orange"))(256)
    gplots::heatmap.2(toPlot, Rowv = TRUE, dendrogram = "column", col = myheatcol, scale = "row", trace = "none", margins = c(7,7),cexRow = cex, cexCol = 0.3 )
