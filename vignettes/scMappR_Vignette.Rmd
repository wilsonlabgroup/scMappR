...
* `cwFoldChange_specificity_tests()`. This function completes a series of statistical tests to measure the cell-type specificity of cwFold-changes. Input a list of cwFold-changes and cell-type proportions from the `case` and `control` conditions. For each gene, cwFold-changes will be compared to a null vector via euclidian distance, average cell-type proportions across all samples, a normal distribution, and a multinomial distribution. For each cell-type, the distribution of cwFold-changes are compared to normal fold-changes.
...
#### Output
  ...
  * non_rank_increase: the same output of pathway enrichment found in `scMappR_and_pathway_analysis`. Please refer to previous section for youtput
...
##### Saved outputs
  ...
  * rank_increase_genes.Rdata: list showing the rank difference between Fold-changes and cwFold-changes for eac cell-type.
...
## Testing confidence and distribution of cwFold-changes.

# THIS NEEDS TO BE FILLED ONCE RESULTS ARE IN
