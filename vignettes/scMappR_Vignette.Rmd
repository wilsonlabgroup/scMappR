...
```{r library_scMappR, include=FALSE, echo = FALSE}

library(scMappR)

```

```{r library_scMappR_eval, eval=FALSE}
...
tissue = "hypothalamus", rda_path = rda_path1, toSave = TRUE, path = "vignette_dir")
...
bulk_DE_cors <- PBMC_example$bulk_DE_cors # 59 sex-specific DEGs in bulk PBMC 
# (up-regulated = female-biased)
...
max_proportion_change <- 10 # maximum cell-type proportion change -- 
#this is good for cell-types that are uncomon in the bulk population. 
# If case has 0 of the cell-type and control has 1%, it is possible that the change in proportion is due to 
# technical variability. To investigate the sudden presence of a rare cell-type, 
# we reccommend using an RNAseq deconvolution tool, not scMappR.

theSpecies <- "human" # these RNA-seq data have human gene symbols (and are also from human)

# To fully utilize scMappR, 
# set internet = TRUE and toSave = TRUE
toOut <- scMappR_and_pathway_analysis(bulk_normalized, odds_ratio_in, 
                                      bulk_DE_cors, case_grep = case_grep,
                                      control_grep = control_grep, rda_path = "", 
                                      max_proportion_change = 10, print_plots = TRUE, 
...
toSave = TRUE, saveSCObject = TRUE, path = "vignette_dir")
...
myheatcol <- grDevices::colorRampPalette(c("lightblue", "white", "orange"))(256)
    gplots::heatmap.2(toPlot, Rowv = TRUE, dendrogram = "column", col = myheatcol, scale = "row",
trace = "none", margins = c(7,7),cexRow = cex, cexCol = 0.3 )
